...
class Web extends Controller
{
    ...
    public function serviceCategory(array $data):void
    {
        ...
        echo $this->view->render("service", [
            "head" => $head,
            "title" => "Serviços de {$category->title}",
    ...
    { 
        ...
        $serviceSearch = (new Service())->find("MATCH(title,subtitle) AGAINST(:s)", "s={$search}");
   ...
   public function  solicitacao(array $data):void
   {
    ...
    if (!empty($data['csrf'])) {
        ...
        if (in_array("", $data)) {
            $json['message'] = $this->message->info("Informe seus dados para efectuar a solicitação.")->render();
            echo json_encode($json);
            return;
        }
        $solicit = (new Request());
        $solicit->auth = $data['author'];
        $solicit->service = $data['service'];
        $solicit->name_client = $data['name'];
        $solicit->email =$data['email'];
        $solicit->sms =$data['content'];
        $solicit->about_service =$data['about_service'];
       

    if (!$solicit->save()) {
            $json['message'] = $this->message->error("Erro ao fazer a solicitação!")->render();
            echo json_encode($json);
            return;
        }else{
            $json['message'] = $this->message->success("Solicitação enviada com sucesso")->render();
            echo json_encode($json);
        }
    }
    ...
    public function login(?array $data): void
    {
        ...
        if (!empty($data['csrf'])) {
            ...
            if(request_limit("weblogin",3,60*5)){
                $json['message'] = $this->message->info("Voçê já efectuou 3 tentativas, esse é o limite. Porfavor, aguarde por 5 minutos para tentar novamente!")->render();
                echo json_encode($json);
                return;
            }
    ...
    public function forget(?array $data)
    {
        ...
        if (!empty($data['csrf'])) {
            ...
            if(request_repeat("webforget",$data["email"])){
                $json['message'] = $this->message->error("Voçê já tentou este email antes")->render();
                echo json_encode($json);
                return;
            }
            $auth = new Auth();
    ...
    public function confirm(): void
    {
        ...
        echo $this->view->render("optin", [
            ...
            "data" => (object)[
                ...
                "desc" => "Enviamos um link de confirmação para seu e-mail. Acesse e siga as instruções para concluir seu cadastro e comece a utilizar o kuatisa",
    ...
    {
        $head = $this->seo->render(
            CONF_SITE_NAME . " - Politicas do Utilizador",
            CONF_SITE_DESC,
            url("/termos"),
            theme("/assets/images/share.jpg")
        );
